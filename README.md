...
- 支持自定义全局快捷键。
- 支持开机自动启动。
- 可独立开启双接入点自动路由分流；两个接入点、识别特征和内网 CIDR 都可在菜单右侧“设置”中维护，内嵌默认值保留现有行为。
...
- `开机自动启动`：登录后自动打开 app。
- `打开自动路由表`：独立监听两个可维护的 macOS 网络服务；两者同时被 App 检测为在线时，当前内网 CIDR 列表走第一个“内网路由出口”，其他流量仍使用 macOS 默认出口。它不受 Server Mode 开关控制。右侧“设置”可选择两个网络服务，并为每端填写可选的 `ipconfig` 摘要识别特征（如 DNS 或 IP）；留空则只判断链路在线。同一窗口也可按一行一条维护 IPv4 CIDR，支持 `/8`、`/16`、`/24`、`/32`，默认保留现有 Wi-Fi（特征 `10.1.20.63`）+ ZTE Mobile Broadband 以及 46 条内嵌规则。保存后会清理旧出口上的 App 托管路由，并按新配置重新对齐；鼠标悬停会显示 App 实时检测到的实际接入点名称、状态、当前路由覆盖数和默认出口。
